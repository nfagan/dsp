function [signals, time_info] = dsp__get_signals(db, SESSIONS)
...
%   SESSIONS = { '1_01262017_Saline' };
%   SESSIONS = { '1_04052016_Saline' };
  % SESSIONS = { '1_02032017_OT' };
  % SESSIONS = get_folder_names( 'H:\SIGNALS\raw\hitch_16_bla' );
end
...
%     plex{end+1} = common_avg;
  ...
  for k = 1:numel(epochs)
    ...
    for j = 1:numel(channels)
      ...
      signal = get_signals( plex{j}, id_times, epoch_events, start_stop ...
...
ids = dsp__get_trial_ids( signals{1} );
...
function all_signals = get_signals( plex, id_times, events, start_stop, w_size, fs )
